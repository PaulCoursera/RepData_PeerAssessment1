...
print(paste("Median steps:", 
      ...
      sep = " ")
```
The mean of steps per day is `r round(mean_steps, 0)`, the median of steps is `r round(median_steps, 0)`.
...
```
Interval `r max_steps[1]` contains the maximum steps. The average number of steps over all the days is: `r  round(max_steps[2])`. 
...
```
The number of NAs in the data set is `r number_na `. This is the same amount as the number of rows with NAs. The only column that contains missing values is "steps". 
The amount of NAs compared to the full dataset is: `r round(number_na/dim(activity)[1]*100,0)` %.
...
```
The mean of steps per day is `r round(mean_noNAs, 0)` the median of steps is `r round(median_noNAs, 0)`. Mean and median value did not change compared to the calculation where the NAs were omitted in the data set. 
